...
from app.heroes import (
    get_archive_protocol,
    extract_heroes_from_details,
    extract_heroes_from_tracker_events,
)
...
class TestReplayExtractorFunctions:
    def test_heroes_extraction_from_details(self, replay):
        with get_archive_protocol(replay) as (archive, protocol):
            ...
            assert len(heroes_details) == 10, print(
                heroes_details, get_replay_details(replay)
            )
    ...
    def test_heroes_extraction_from_tracker_events(self, replay):
        with get_archive_protocol(replay) as (archive, protocol):
            heroes_details = extract_heroes_from_tracker_events(
                archive, protocol, False
            )
    ...
    def test_hero_sets_equal(self, replay):
        ...
        if "details" not in pytest.shared.get(replay, {}):
            try:
                ...
                pass
        assert sorted(pytest.shared[replay]["details"]) == sorted(
            pytest.shared[replay]["tracker_events"]
        ), print(get_replay_details(replay))
