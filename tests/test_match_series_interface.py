...
def test_just_works(pre_bans, session, match_series_list):
    for index, match_series in enumerate(pre_bans.items()):
        ...
        assert manager.match_series.banned_heroes == match_series[1]
...
def test_bans(pre_bans, session, match_series_list):
    ...
    assert manager.match_series.banned_heroes == {"rexxar"}

    new_manager = MatchSeriesManager(session, match_series_list[0].id)
    assert new_manager.match_series.banned_heroes == {"rexxar"}
    ...
    assert manager.match_series.banned_heroes == {"anduin", "probius", "blaze"}
...
def test_unbans(pre_bans, session, match_series_list):
    ...
    assert manager.match_series.banned_heroes == {"deathwing"}
...
def test_bans_unbans(pre_bans, session, match_series_list):
    ...
    assert manager.match_series.banned_heroes == {"deathwing", "anduin"}
    ...
    assert manager.match_series.banned_heroes == {"anduin"}
...
def test_cho_gall(pre_bans, session, match_series_list):
    ...
    assert manager.match_series.banned_heroes == {
        "deathwing",
        "brightwing",
        "cho",
        "gall",
    ...
    assert manager.match_series.banned_heroes == {"deathwing", "brightwing", "blaze"}
